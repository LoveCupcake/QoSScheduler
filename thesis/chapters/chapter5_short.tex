...
\subsection{Scenario 1: Bandwidth Throttling Precision}

\textbf{Objective:} Verify that the Token Bucket accurately enforces a configured bandwidth cap.

\textbf{Procedure:} Configure a 5~Mbps cap for the iPerf3 application. Generate 10~Mbps TCP traffic (client on Android, server on Ubuntu). Measure actual throughput over 60 seconds. Repeat 10 times and compute mean, standard deviation, and 95\% confidence interval.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{figures/real_token_bucket.png}
\caption{Token Bucket enforcement in action: aggressive throttling of low-priority background processes versus full allocation for foreground apps.}
...
\subsection{Scenario 3: Bufferbloat Mitigation}

\textbf{Objective:} Measure the latency reduction achieved during network congestion.

\textbf{Procedure:} \textit{Baseline:} Generate saturating background traffic while measuring round-trip latency with \texttt{ping -c 100 8.8.8.8} (QoS disabled). \textit{Treatment:} Repeat with QoS enabled, background traffic set to LOW priority. Compare latencies and compute statistical significance.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{figures/real_drop_rate.png}
\caption{Packet Drop Rate dynamics during network congestion. The X-shape cross-over demonstrates Token Bucket effectively suppressing greedy flows to relieve Bufferbloat.}
\label{fig:bufferbloat_ping}
\end{figure}
